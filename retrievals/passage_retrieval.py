...
def main(args):
    ...
    for d in tqdm(data , desc="doing retrieval..."):
        try:
            result = retriever.search_document(d, args.n_docs)
            results.append({'question': d, 'contexts' : result})
        except:
            continue
    with open(args.output_dir, 'wb') as handle:
